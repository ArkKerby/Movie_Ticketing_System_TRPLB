...
if ($hasCinemaAssignments) {
  // Show movies that are now_showing=1 OR assigned to at least one cinema number
  $nowShowingQuery = $conn->query("
        ...
        FROM MOVIE m
        LEFT JOIN CINEMA_MOVIE_ASSIGNMENT cma ON m.movie_show_id = cma.movie_show_id
        WHERE (m.is_deleted = 0 OR m.is_deleted IS NULL)
          AND (m.now_showing = 1 OR cma.assignment_id IS NOT NULL)
  ...
  // Fallback: no cinema assignments table, just use now_showing flag
  $nowShowingQuery = $conn->query("
        SELECT m.movie_show_id, m.title, m.genre, m.duration, m.rating, m.movie_descrp, m.image_poster{$carousel_select}, m.delete_at{$trailer_select}
        FROM MOVIE m
        WHERE (m.is_deleted = 0 OR m.is_deleted IS NULL)
          AND m.now_showing = 1
        ORDER BY m.title ASC
...
<body>
  ...
  <?php if ($showPwdBannerHome): ?>
    ...
    opacity:0;
  ">
      <span>Have you submitted your PWD ID to have a 20% discount? <a href="profile.php#pwd"
          style="color:#7ab5ff;font-weight:600;text-decoration:underline;">Apply on your Profile.</a></span>
      <button onclick="dismissPwdToast()"
        style="background:none;border:none;color:rgba(255,255,255,0.45);cursor:pointer;font-size:1rem;padding:0 2px;line-height:1;flex-shrink:0;">x</button>
    </div>
    <script>
      (function () {
  ...
  <header>
    ...
    <div class="right-section">
      <?php if (isset($_SESSION['logged_in']) && $_SESSION['logged_in']): ?>
        <a href="branch-selection.php" class="ticket-btn"
          style="text-decoration: none; display: inline-block; color: white; padding: 10px 25px; border-radius: 25px; cursor: pointer;">Buy
          Tickets</a>
      <?php else: ?>
        <a href="login.php" class="ticket-btn"
          style="text-decoration: none; display: inline-block; color: white; padding: 10px 25px; border-radius: 25px; cursor: pointer;">Buy
          Tickets</a>
      <?php endif; ?>
      <?php if (isset($_SESSION['logged_in']) && $_SESSION['logged_in']): ?>
        ...
        foreach ($carouselMovies as $movie):
          ...
          <div class="hero-slide <?php echo $slideIndex === 1 ? 'active' : ''; ?>" data-trailer="<?php echo $trailerId; ?>">
            ...
            <div class="hero-trailer-container" style="display: none;">
              <iframe class="hero-trailer-iframe" src="" frameborder="0" allow="autoplay; encrypted-media"
      ...
      <div class="slide-indicators">
        <?php if (!empty($nowShowingMovies)): ?>
          ...
          foreach ($carouselMovies as $movie):
            $indicatorIndex++;
            ?>
            <span class="indicator <?php echo $indicatorIndex === 1 ? 'active' : ''; ?>"
              onclick="currentSlide(<?php echo $indicatorIndex; ?>)"></span>
          <?php endforeach; ?>
        <?php else: ?>
  ...
  <div class="main-sections-wrapper">
      ...
      <div class="movie-grid">
        <?php if (count($nowShowingMovies) > 0): ?>
          <?php foreach ($nowShowingMovies as $movie):
            ...
            ?>
            <div class="movie" onclick="openMovieModal(this)" data-title="<?= $title ?>" data-genre="<?= $genre ?>"
              data-duration="<?= $duration_formatted ?>" data-rating="<?= $rating ?>" data-poster="<?= $image ?>"
              data-description="<?= $description ?>" data-delete="<?= htmlspecialchars($deleteOn) ?>"
              data-trailer="<?= $trailerAttr ?>">
                ...
                <div class="movie-info">
                  ...
                  <?php if (!empty($movieCinemaMap[$movie['movie_show_id']])): ?>
                    <div class="cinema-badges">
                      <?php foreach ($movieCinemaMap[$movie['movie_show_id']] as $cinemaName): ?>
                        <span
                          class="cinema-badge cinema-badge--<?= strtolower(preg_replace('/[^a-zA-Z]/', '', $cinemaName)) ?>"><?= htmlspecialchars($cinemaName) ?></span>
                      <?php endforeach; ?>
                    </div>
                  <?php endif; ?>
                  ...
                  <div class="movie-actions">
                    <button class="action-btn trailer-btn"
                      onclick="event.stopPropagation(); openTrailer('<?= $title ?>', '<?= $trailerAttr ?>')">▶
                      Trailer</button>
                    <a href="branch-selection.php?source=movie&movie=<?= urlencode($title) ?>" class="action-btn ticket-btn"
                      style="text-decoration: none; display: inline-block;"
                      onclick="event.stopPropagation(); return true;">Buy Tickets</a>
                  </div>
                </div>
              </div>
            </div>
          <?php endforeach; ?>
        <?php else: ?>
          <p style="text-align: center; color: #ccc; grid-column: 1 / -1; padding: 40px;">No movies currently showing.
            Check back soon!</p>
        <?php endif; ?>
      </div>
    </section>
            ...
            <div class="movie">
              ...
              <div class="movie-info">
                ...
                <button class="notify-btn"
                  onclick="alert('We will notify you when <?= $title ?> becomes available!')">Notify Me</button>
              </div>
            </div>
          <?php endforeach; ?>
        <?php else: ?>
          <p style="text-align: center; color: #ccc; grid-column: 1 / -1; padding: 40px;">No upcoming movies. Check back
            soon!</p>
        <?php endif; ?>
      </div>
    </section>
    ...
    <section id="portal-access" style="
    background: linear-gradient(180deg, transparent 0%, rgba(5,15,35,0.95) 100%);
    padding: 48px 20px 56px;
    text-align: center;
    border-top: 1px solid rgba(85,138,206,0.12);
  ">
      <p style="
      font-size: 11px;
      font-weight: 700;
    ...
    ">Internal Access</p>
      <div style="display: flex; justify-content: center; gap: 16px; flex-wrap: wrap;">
        <!-- Staff Login Button -->
        <a href="staff-login.php" id="staff-portal-btn" style="
        display: inline-flex; align-items: center; gap: 10px;
        padding: 12px 26px;
        background: rgba(85,138,206,0.12);
        ...
        cursor: pointer;
      " onmouseover="this.style.background='rgba(85,138,206,0.22)';this.style.borderColor='rgba(85,138,206,0.6)';this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 20px rgba(85,138,206,0.25)';"
          onmouseout="this.style.background='rgba(85,138,206,0.12)';this.style.borderColor='rgba(85,138,206,0.35)';this.style.transform='translateY(0)';this.style.boxShadow='none';">
          <span style="font-size:16px;"></span> Staff Login
        </a>

        <!-- Admin Login Button -->
        <a href="admin-login.php" id="admin-portal-btn" style="
        display: inline-flex; align-items: center; gap: 10px;
        padding: 12px 26px;
        ...
        transition: background 0.22s, border-color 0.22s, transform 0.18s, box-shadow 0.22s;
        cursor: pointer;
      " onmouseover="this.style.background='rgba(160,100,220,0.22)';this.style.borderColor='rgba(160,100,220,0.6)';this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 20px rgba(160,100,220,0.25)';"
          onmouseout="this.style.background='rgba(160,100,220,0.12)';this.style.borderColor='rgba(160,100,220,0.35)';this.style.transform='translateY(0)';this.style.boxShadow='none';">
          <span style="font-size:16px;"></span> Admin Login
        </a>
      </div>
    </section>
      ...
      <div class="movie-detail-content">
        ...
        <div class="movie-details">
          ...
          <div class="movie-description">
            <p id="modalMovieDescription">Experience the ultimate cinematic adventure with stunning visuals and an
              unforgettable story.</p>
          </div>
          <div class="modal-actions">
            <!-- Removed the Watch Trailer button -->
            <a href="#" class="action-btn ticket-btn" onclick="goToSeatReservation(); return false;"
              style="text-decoration: none; display: inline-block;">
              Buy Tickets
            </a>
          </div>
  ...
  <footer>
    ...
    <div class="footer-right">
      ...
      <div class="social-icons">
        <a href="https://www.facebook.com/photo?fbid=110530527255536&set=a.110530550588867"><img
        ...
        <a href="https://www.facebook.com/photo?fbid=110530527255536&set=a.110530550588867"><img src="images/x.png"
            alt="X"></a>
        <a href="https://www.facebook.com/photo?fbid=110530527255536&set=a.110530550588867"><img src="images/tiktok.png"
            alt="TikTok"></a>
        <a href="https://www.facebook.com/photo?fbid=110530527255536&set=a.110530550588867"><img
  ...
  <script>
    (function () {
      ...
      // Intercept all links going to login.php
      document.addEventListener('click', function (e) {
        var link = e.target.closest('a[href*="login.php"]');
        if (!link) return;
