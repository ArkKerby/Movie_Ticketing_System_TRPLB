...
if (isset($_SESSION['logged_in']) && $_SESSION['logged_in'] && isset($_SESSION['role']) && $_SESSION['role'] === 'mall_admin') {
  ...
  if (!$email || !$password) {
    ...
    if ($result && $result->num_rows > 0) {
      ...
      if ($row['role'] !== 'mall_admin') {
        ...
        if (!$verified) {
          $error_message = "Account not verified. Please contact the system administrator.";
        } else {
          $_SESSION = [];
      ...
      $error_message = "No administrator account found with that email.";
    }
...
<head>
  ...
  <title>Admin Portal – Ticketix</title>
  ...
  <style>
    ...
    * {
    ...
    .login-card {
      ...
      border-radius: 18px;
      padding: 40px 36px;
      box-shadow: 0 16px 48px rgba(0, 0, 0, 0.55), 0 0 40px rgba(160, 100, 220, 0.15);
    }
    ...
    .form-control:focus {
      ...
      box-shadow: 0 0 0 3px rgba(160, 100, 220, 0.2);
    }
    ...
    .btn-login {
      width: 100%;
      padding: 14px;
      background: linear-gradient(135deg, #a064dc, #7040b0);
      border: none;
      border-radius: 8px;
      color: #fff;
      font-family: 'Montserrat', sans-serif;
      ...
      box-shadow: 0 4px 16px rgba(160, 100, 220, 0.35);
    }

    .btn-login:hover {
      background: linear-gradient(135deg, #b87ef0, #8050c8);
      transform: translateY(-1px);
      box-shadow: 0 6px 20px rgba(160, 100, 220, 0.5);
    }
  ...
  <div class="login-wrapper">
    ...
    <div class="login-card">
      <div class="login-header">
        ...
        <p class="login-subtitle">Sign in to manage the Ticketix platform</p>
      </div>
      ...
      <form method="POST" action="admin-login.php" autocomplete="off">
        <div class="form-group">
          ...
          <input type="email" id="email" name="email" class="form-control" placeholder="admin@ticketix.com"
        ...
        <button type="submit" class="btn-login">Sign In to Admin Portal</button>
      </form>

      <p class="login-note"> Authorized administrators only</p>
    </div>
  </div>
